...
(defn lcg-next [[a b] [ax bx]]
  [(mod (* a ax) D), (mod (+ (* a bx) b) D)])

(defn parse-line [line stack-fn cut-fn inc-fn]
  (let [[a b _ d] (str/split line #" ")]
    (if (= a "cut")
      (cut-fn (Long/parseLong b))
      (if (= b "with")
        (inc-fn (Long/parseLong d))
        stack-fn))))

; pt.1
(binding [D 10007]
  (let [shuffles (map #(parse-line % deal-stack cut-deck-of deal-inc-of) (input-lines 2019 22))
        F        (apply comp (reverse shuffles))]
    (lcg-next (F [1 0]) [0 2019])))
...
; modInv는 내장함수를 이용하였음.
;
; (F^-1)^R 에서 R은 매우 큰 값이 나오는데, F^-1도 LCG에 대한 호모몰피즘임을 고려할 때
...
(defn inv-cut-deck-of [n]
  (cut-deck-of (- n)))

(defn inv-deal-inc-of [n]
  (deal-inc-of (mod-inv n)))

(defn lcg-nth [n [a b]]
  [(mod-exp a n), (mod (* b (* (- (mod-exp a n) 1) (mod-inv (- a 1)))) D)])

(let [shuffles (map #(parse-line % inv-deal-stack inv-cut-deck-of inv-deal-inc-of) (input-lines 2019 22))
      inv-F    (apply comp shuffles)]
  (lcg-next (lcg-nth R (inv-F [1 0])) [0 2020]))
