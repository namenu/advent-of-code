...
(s/def ::graph #(satisfies? g/IDirectedGraph %))
(s/def ::tasks (s/map-of char? int?))
(s/def ::num-workers pos-int?)
(s/def ::doing set?)
(s/def ::done vector?)
(s/def ::tick int?)

(s/def ::state (s/keys :req-un [::graph ::tasks ::num-workers ::doing ::done ::tick]))
...
(comment
  (let [input ["Step C must be finished before step A can begin."
    ...
    (s/valid? ::state (init-state (input->graph input)))
